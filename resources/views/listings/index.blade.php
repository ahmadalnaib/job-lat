...
<section class="container px-5 py-12 mx-auto">
  ...
  <div class="mb-12">
    <div class="flex-justify-center">
      @foreach ($categories as $category)
          <a href="{{route('categories.jobs',$category->slug)}}" class="inline-block ml-2 tracking-wide text-xs font-medium title-font py-0.5 px-1.5 border border-indigo-500 uppercase }}">
            {{$category->name}}
          </a>
      @endforeach
    </div>
  ...
  <div class="mb-12">
    <h2 class="text-2xl fontmedium text-gray-900 title-font px-4">All Jobs</h2>
  </div>
  <div class="-my-6 ">
    @foreach ($listings as $listing)
        <a href="{{route('listings.show',$listing->slug)}}" class="py-6 px-4 flex flex-wrap md:flex-nowrap border-b border-gray-100 {{$listing->is_highlighted ? 'bg-yellow-100 hover:bg-yellow-200 ':'bg-white hover:bg-gray-100'}}">
          <div class="md-w16 md:mb-0 mb-6 mr-4 flex-shrink-0 flex flex-col">
          ...
          <h2 class="text-xl font-bold text-gray-900 title-font mb-1">{{$listing->title}}</h2>
            ...
            <p class="leading-relaxed text-gray-900">{{$listing->company}} &mdash;
               <span class="text-yellow-500"> {{$listing->location}} </span>
              </p>
          </div>
